...
exports.getGlobal = function(roomData = {}) {
   ...
   if (allGames[getGlobalName()] && !allGames[getGlobalName()]._isGameStarted)
      return allGames[getGlobalName()];
   globalGameNum++;
...
function route(client, mess) {
   ...
   if (allGames[room_id]) {
      try {
         if (typeof allGames[room_id].hook === 'function')
            allGames[room_id].hook(client.id, mess);
      } catch (e) {
         console.log(e);
         client.emit('debug', {
            status: 'ISR',
